...
import { FallbackFunctions,FallbackFunctions__factory,Testfunction } from "../typechain-types";
import { Testfunction__factory} from "../typechain-types";
...
async function main() {
  ...
  const FallbackFunctions:FallbackFunctions__factory = await ethers.getContractFactory("FallbackFunctions");
   ...
   const Testfunction:Testfunction__factory = await ethers.getContractFactory("Testfunction");
   const et:Testfunction = await Testfunction.deploy();
   console.log("et.address= ", et.address);
   await et.deployed();

   console.log("FallbackFunctions deployed to ", e.address);
   console.log("Balance of contract",  (await ethers.provider.getBalance(e.address)).toString());
   ...
   const txt1 = await et.TestfunctionCall(e.address,"addSome()");
